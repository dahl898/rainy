  ...
  const getSmoothPath = (chartCoordinates) => {
    ...
    for (let i = 1; i < chartCoordinates.length; i++) {
      ...
      const cps = controlPoint(
        chartCoordinates[i - 1],
        chartCoordinates[i - 2],
        chartCoordinates[i]
      )
      ...
      d.push(
        `C ${cps.x},${cps.y} ${cpe.x},${cpe.y} ${chartCoordinates[i].x},${chartCoordinates[i].y}`
      )
      // }
    }
    return d.join(' ')
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width={width} height={height}>
      <defs>
        ...
        <linearGradient
          id="chartGradient_precipitation"
          x1="0%"
          y1="0%"
          x2="0%"
          y2="100%"
